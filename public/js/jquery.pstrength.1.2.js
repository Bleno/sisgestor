...
(function($){
	$.extend($.fn, {
		pstrength : function(options) {
			var options = $.extend({
				verdects:	["Muito Fraco","Fraco, insira números ou caractéres","Média","Forte","Muito Forte"],
			...
			return this.each(function(){
				...
				//$(this).after('<div class="pstrength-minchar" id="' + infoarea + '_minchar"><br>Numero minimo de caractéres e ' + options.minchar + '</div>');
				$(this).after('<div class="pstrength-info" id="' + infoarea + '_text" style="float: left; margin-left: -161px" ></div>');
				$(this).after('<div class="pstrength-bar" id="' + infoarea + '_bar" style="border: 0px solid white; font-size: 1px; height: 4px; width: 0px;  margin-left: 3px; margin-top: 0px;;"></div>');
				$(this).keyup(function(){
		...
		runPassword : function (password, infoarea, options){
			...
			if (nPerc == -200) {
				strColor = '#dc143c';
				strText = 'Senha muito muito fraca !';
				$(ctlBar).css({width: "2%"});
			}
			else if (nPerc < 0 && nPerc > -199) {
				strColor = '#a52a2a';
				strText = 'Muito Pequeno';
				$(ctlBar).css({width: "12%"});
			}
			...
			$(ctlText).html("<span id='forca' style='color: " + strColor + "; margin-left: 164px;'>" + strText + "</span>");
		},
